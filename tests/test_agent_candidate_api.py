...
def test_candidate_api_runs_candidate_model_class(tmp_path):
    candidate = tmp_path / "candidate_class.py"
    candidate.write_text(
        """
import pandas as pd

class CandidateModel:
    def fit(self, train_metadata, train_cycle_summary, train_labels, config):
        return float(train_labels['y'].mean())

    def predict(self, test_metadata, test_cycle_summary, config):
        return pd.DataFrame({'row_id': test_metadata['row_id'], 'y_pred': 111.0})
"""
    )
    meta = pd.DataFrame({"row_id": [1, 2]})
    cycles = pd.DataFrame({"row_id": [1, 2], "cycle_index": [1, 1], "discharge_capacity": [1.0, 1.0]})
    ...
    assert result.success
    assert result.predictions is not None
    assert result.predictions["y_pred"].tolist() == [111.0, 111.0]
